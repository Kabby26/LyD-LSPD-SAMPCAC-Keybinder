...
Gui, Add, Text, x2 y39 w470 h310 , - /ov `t`t`t`t`tIngame Overlay an-/ausschalten`n/laufen `t`t`t`t`tLaufscript an-/ausschalten`n`nWeitere Hotkeys:`nM: `t`t`t`t`t/dienst`t`t`t`t`t/Waffenspint`nAlt + K: `t`t`t`t`t/finden`n`nP: `t`t`t`t`t/flock`nJ: `t`t`t`t`t/hsirene`nY: `t`t`t`t`tBleiben Sie stehen!`nR: `t`t`t`t`tDurchsuchen`nX: `t`t`t`t`tVerhaften`n`nNumpad0: `t`t`t`t/pdgate`t`t`t`t/agate`nNumpad1: `t`t`t`tTimer starten`nNumpad2: `t`t`t`tTimer beenden`n`n
Gui, Add, Button, x312 y349 w160 h30 gAbord, Zurück
...
if(IsPlayerInRange3D(1027.2971,-1616.1125,5.9308, 3) && isPlayerDriver()){
...
Sleep 2000
SendChat("/fc Carrespawn in 3 Sekunden!")
Sleep 1000
...
if(IsPlayerInRange3D(2883.5112,1980.7412,11.2656, 10) || IsPlayerInRange3D(2878.2378,1934.4117,11.2550, 10)){
	SendChat("/agate")
}else{
	SendChat("/pdgate")
}
...
if(IsPlayerInRange3D(1638.9738,-1381.8706,1962.5164, 2)){
	SendChat("/dienst")
	SendChat("/copman")
	SendChat("/rank")
}
if(IsPlayerInRange3D(1644.4296,-1384.8109,1962.5164, 2)){
...
if(IsPlayerInRange3D(1027.4275,-1672.2449,5.9308, 2)){
	SendChat("Los Santos Police Department!")
	SendChat("Sie sind verhaftet!")
	SendInput t/arrest{space}
}
...
:?:/eventmsg::
SendChat("/gc - - - Event: Weihnachtliche Purge - - -")
SendChat("/gc 21.12.2018 um 20:00 Uhr")
SendChat("/gc Noch kurz vor Weihnachten hat die Regierung von San Andreas eine Purge angekündigt.")
SendChat("/gc Diese wird komplett in Las Venturas stattfinden.")
SendChat("/gc Kämpft um euer Überleben, damit ihr noch das Fest mitbekommt.")
SendChat("/gc Teilnahmebedingungen: Level 3, Genug Kapital für Waffen")
SendChat("/gc Dauer: 60 Minuten")
return

:?:/purgemsgrp::
SendChat("/smsg Wir, die Regierung von San Andreas, verkünden den Termin...")
SendChat("/smsg ... für die alljaehrliche Pruge.")
SendChat("/smsg Die Purge wird heute Abend ab 20:00 Uhr in Las Venturas stattfinden.")
SendChat("/smsg Sanitätern wurde Immunität gewährt, diese dürfen nicht verletzt werden!")
SendChat("/smsg Sobald um 20:00 Uhr die Sirenen ertönen, sind sämtliche Verbrechen...")
SendChat("/smsg ... in Las Venturas, für eine Stunde erlaubt!")
SendChat("/smsg Gott schütze Amerika!")
return
